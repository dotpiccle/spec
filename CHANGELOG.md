...
- Finite mandatory qualification matrix for reverb conformance at `test-vectors/numeric/reverb-qualification-matrix.json`. The matrix defines 10 non-canonical `(tail_ms, soften_hz, sample_rate)` configurations plus a representative nine-rate set for continuous sample-rate APIs. Combined with the 5 canonical fixtures, matrix-construction vector, and each declared discrete numeric/channel/storage mode, this gives a finite conformance bar. Added terminology distinguishing "document reverb configuration" (`tail_ms`, `soften_hz`) from "render profile" (`sample_rate`, numeric mode) from "reference IR configuration" (the combination). Property-based differential testing is recommended (SHOULD) with ≥100 random tuples and an engine-chosen seed; results are informative and do not affect conformance.
...
- **Modal resonance floor tolerance tightened from one-sided to hybrid.** The previous one-sided `engine ≤ ref + 6 dB` is supplemented with an absolute floor `engine ≤ −25 dB` (derived from the worst non-degenerate reference fixture, `−27.7 dB` for `tail_ms = 20`, plus 2.7 dB headroom, rounded to `−25 dB`). Both clauses applied to the canonical references. The floor was later tightened to `−30 dB` and made conditional when a valid same-configuration reference is itself above it.
...
- Made the `−30 dB` modal quality floor conditional on the same-configuration reference meeting
  that floor, so valid low-rate references cannot fail against bit-identical engines.
- Replaced impossible exact equality for `sin`/`cos`-derived balance and biquad aids with an
  explicit scaled binary64-epsilon tolerance, and regenerated center downmix from the normative
  balance formula.
- Corrected the valid-fixture inventory in the engine build guide from 38 documents to 37.
- Replaced exhaustive qualification of continuous sample-rate APIs with a finite representative
  sample-rate matrix while keeping discrete numeric/channel/storage modes exhaustive.
