...
The numeric and behavior aids (`dsp-values.json`, `render-cases.json`, the reverb reference IR fixtures, and the reverb matrix test vector) are **non-normative as format definition** — they are derivable from the formulas in this specification and can be regenerated from them. However, they are **normatively referenced** by the conformance gates in [Engine Build Guide](15-engine-build-guide.md): a conforming engine MUST reproduce their canonical-mode values exactly, except where a gate provides an alternate check for entries subject to permitted transcendental variance (see [Engine Safety](11-engine-safety.md)). If any value in these fixtures is found to disagree with the formulas in `docs/`, the formulas are authoritative; the fixture is corrected in place; and engines must then match the corrected fixture. The repository validator recomputes every numeric aid from the formulas and asserts equality, so drift is caught mechanically.
