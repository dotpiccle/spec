...
6. **Verify reverb cross-engine equivalence.** A conforming engine MUST pass all of the following:
   ...
   d. **Additional render profiles**: For each additional declared render profile (other sample rates, binary32 mode, etc.), render the 5 canonical document reverb configurations (`tail_ms ∈ {1, 10, 20, 220, 500}`, `soften_hz = 4000`) at that profile's sample rate, generate the reference IR on demand, and assert the seven tolerances.
   ...
   This finite mandatory set (a–d) plus the recommended property-based pass (e) replaces the earlier "for each declared reverb configuration" language, which was not finitely satisfiable. An engine that passes (a–d) can honestly claim reverb qualification for its declared render profiles.
