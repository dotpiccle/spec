...
6. **Verify reverb and echo cross-engine equivalence.** A conforming engine MUST pass all of the following:
   ...
   c. **Stacked-effects fixtures** (2 configurations): Render `test-vectors/valid/spatial-effects-reverb-then-echo.json` and `test-vectors/valid/spatial-effects-echo-then-reverb.json` in canonical mode. Verify that the two orders produce different output (reverb crossfade and echo additive wet are non-commutative), that each stage processes the preceding stage's entire output, and that the output terminates at the cumulative boundary `Eₙ`. These fixtures prove that array order is a normative behavior.
   ...
   e. **Matrix construction vector** (1 configuration): Compare your `Q` against the test vector in [test-vectors/numeric/reverb-matrix-vector.json](../test-vectors/numeric/reverb-matrix-vector.json) for `tail_ms=37`, `soften_hz=8000`.
...
7. **Assert finite output and exact output-frame counts.** Compute the document's expected total frame count from the accumulated per-stage frame boundary `Eₙ` (where `E₀ = frame(D)` and each effect's `tail_frames` is computed in frames from its own parameters — see [Spatial Effects](07-spatial-effects.md) §Stage boundaries), then verify the rendered output has exactly that many frames and no non-finite (NaN, infinity) samples.
